...
class FairModelWrapper(keras.Model):
    ...
    Note:
        * **Input protocol.** During training the wrapper expects inputs
          of the form ``(X, q)`` (a two-tuple).  During inference it
          accepts ``X`` alone.  :meth:`fit` and :meth:`predict` handle
          the packing / unpacking automatically so the user sees a flat
          ``fit(X, y, q=q)`` / ``predict(X)`` surface.
    ...
    def call(self, inputs, training=False):
        """Forward pass; adds the fairness penalty when ``inputs`` is ``(X, q)``.

        Args:
            inputs: Either a tensor ``X`` (inference) or a two-tuple
                ``(X, q)`` (training with fairness).  When ``q`` is
                provided and ``mu > 0``, the scaled fairness penalty is
                added to the model losses via :meth:`add_loss`.
            training (bool): Forwarded to the base model.
