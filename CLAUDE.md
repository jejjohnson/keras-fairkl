...
Fairness-constrained kernel learning for Keras 3. HSIC/MMD penalties, exact and approximate kernels, and fair models.
...
make typecheck            # Type check: ty check src/fairkl
...
uv run --group lint ruff check .              # Lint — ENTIRE repo, not just src/fairkl/
uv run --group lint ruff format --check .     # Format — ENTIRE repo
uv run --group typecheck ty check src/fairkl  # Typecheck — package only
```

**Critical**: Always lint/format with `.` (repo root), not `src/fairkl/`. CI runs `ruff check .` which includes `tests/` and `scripts/`.

## Architecture

### Package structure

All implementation lives in `src/fairkl/`. The public API is re-exported through `src/fairkl/__init__.py`.

### Key directories

| Path | Purpose |
|------|---------|
| `src/fairkl/` | Main package source code |
| `tests/` | Test suite |
