...
**All four checks must pass before any commit.** CI runs them on the entire repo (`ruff check .`), not just `src/fairkl/`, so always run the commands below from the repo root.
...
uv run --group typecheck ty check src/fairkl
```

> **Common pitfall**: Running `ruff check src/fairkl/` instead of `ruff check .` misses import-sorting errors in `tests/` and `scripts/`. The CI workflow runs `ruff check .`. Always use `.` (repo root), not a subdirectory.
